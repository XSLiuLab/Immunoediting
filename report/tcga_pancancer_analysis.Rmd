...
sim_2000_not_filter$cancer <- get_cancer_type(sim_2000_not_filter$sample,
                                              parallel = T,cores = 20)
saveRDS(sim_2000_not_filter,file = "../data/sim_2000_not_filter_all_ccf.rds")
```
...
```{r}
sim_all <- readRDS("../data/sim_2000_all_ccf.rds")
sim_all %>%
  group_by(cancer,sim_num) %>%
  summarise(median_es=median(es)) -> summ2
...
```{r}
sim_all <- readRDS("../data/sim_2000_all_exp.rds")
